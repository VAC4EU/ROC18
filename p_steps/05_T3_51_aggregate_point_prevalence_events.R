...
for (subpop in subpopulations_non_empty) {  
  ...
  load(paste0(diroutput, "D4_count_events_point_prevalence", suffix[[subpop]], ".RData"))
  PR_yearly <- get(paste0("D4_count_events_point_prevalence", suffix[[subpop]]))
  rm(list = paste0("D4_count_events_point_prevalence", suffix[[subpop]]))
  
  # Find the columns for counts and PT
  cols_to_sums <- names(PR_yearly)[grepl("^prev_|^in_population$", names(PR_yearly))]
  ...
  setnames(PR_yearly, cols_to_change_name, gsub("^prev_", "num_point_", cols_to_change_name))
  
  PR_yearly[, year := as.character(year(as.Date(substr(timeframe, 1, 10))))]
  PR_yearly[, timeframe := NULL]
  
  setnames(PR_yearly, "in_population", "denominator_point")
  setcolorder(PR_yearly, c("sex", "Ageband", "year", "denominator_point"))
  
  nameoutput <- paste0("D4_events_point_prevalence_aggregated", suffix[[subpop]])
