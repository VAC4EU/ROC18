...
for (subpop in subpopulations_non_empty) {
  ...
  TD_variables <- unique(c(OUTCOME_variables, SECCOMPONENTS))
  ...
  single_day <- days(1)
  complete_TD <- complete_TD[, if(.N > 1 || value_of_variable == 1) .SD, by = c("person_id", "variable")]
  complete_TD[, date_end := data.table::shift(date, type = "lead"), by = c("person_id", "variable")]
  ...
  complete_TD <- complete_TD[value_of_variable == 1, ][, value_of_variable := NULL]
  
  setcolorder(complete_TD, c("person_id", "date", "date_end"))
  
  complete_TD <- rbindlist(list(
    complete_TD,
    get(load(paste0(direvents, "D3_events_DEATH.RData"))[[1]])[, date_end := date][, variable := "DEATH"]
  ))
  
  # Export final dataset
  name_export_df <- paste0("D3_TD_events_complete", suffix[[subpop]])
